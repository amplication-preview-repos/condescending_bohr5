...
export class NodeResolverBase {
  ...
  async createNode(@graphql.Args() args: CreateNodeArgs): Promise<Node> {
    return await this.service.createNode({
      ...
      data: {
        ...
        edge: args.data.edge
          ? {
              connect: args.data.edge,
            }
          : undefined,

        flow: args.data.flow
          ? {
              connect: args.data.flow,
            }
          : undefined,
      },
    });
  ...
  @graphql.ResolveField(() => Edge, {
    nullable: true,
    name: "edge",
  })
  async getEdge(@graphql.Parent() parent: Node): Promise<Edge | null> {
    const result = await this.service.getEdge(parent.id);
  ...
  @graphql.ResolveField(() => Flow, {
    nullable: true,
    name: "flow",
  })
  async getFlow(@graphql.Parent() parent: Node): Promise<Flow | null> {
    const result = await this.service.getFlow(parent.id);
